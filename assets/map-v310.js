...
(() => {
  ...
  async function loadWorld(){
    try{const d=await fetch('./data/world-v310.geojson',{cache:'force-cache'});if(!d.ok)throw new Error('world');s.world=await d.json()}catch(e){s.world=null}
  ...
  function redrawStatic(){
    ...
    for(const e of conflicts){const q=ellipseFor(e);if(!q)continue;ctx.save();ctx.beginPath();ctx.ellipse(q.x,q.y,q.rx,q.ry,0,0,Math.PI*2);ctx.fillStyle=colorMix(color(e),.045);ctx.strokeStyle=colorMix(color(e),.23);ctx.lineWidth=1;ctx.setLineDash([5,8]);ctx.fill();ctx.stroke();ctx.restore();s.hit.push({e,x:q.x,y:q.y,r:14})}
  ...
  function drawMarker(ctx,e,x,y){
    const c=color(e),sev=e.severity||1,r=sev>=5?7:sev>=4?6:sev>=3?5:4;ctx.save();ctx.strokeStyle=c;ctx.fillStyle=c;ctx.lineWidth=sev>=4?1.9:1.25;
    if(sev>=4){ctx.beginPath();ctx.arc(x,y,r+5,0,Math.PI*2);ctx.fillStyle=colorMix(c,.09);ctx.fill();ctx.fillStyle=c}
    if(e.category==='security'){ctx.translate(x,y);ctx.rotate(Math.PI/4);ctx.fillRect(-r*.72,-r*.72,r*1.44,r*1.44)}
    else if(e.category==='wildfire'){ctx.beginPath();ctx.moveTo(x,y-r);ctx.lineTo(x+r*.9,y+r*.8);ctx.lineTo(x-r*.9,y+r*.8);ctx.closePath();ctx.fill()}
    else if(e.category==='earthquake'){ctx.beginPath();ctx.arc(x,y,r,0,Math.PI*2);ctx.stroke()}
    else if(e.category==='space'){star(ctx,x,y,r+1,c)}
    else{ctx.beginPath();ctx.arc(x,y,r,0,Math.PI*2);ctx.fill()}
    if(e.id===s.selectedId){ctx.beginPath();ctx.arc(x,y,r+7,0,Math.PI*2);ctx.strokeStyle=css('ink');ctx.lineWidth=1.4;ctx.stroke()}
    ctx.restore();
  }
  ...
  function buildAnimated(){
    const live=s.visible.filter(e=>e.severity>=3&&['storm','weather','earthquake','security','wildfire','flood'].includes(e.category)).sort((a,b)=>score(b)-score(a));
    const out=[],counts={};for(const e of live){const cap=e.category==='weather'?3:e.category==='conflict'?4:2;if((counts[e.category]||0)>=cap)continue;counts[e.category]=(counts[e.category]||0)+1;out.push(e);if(out.length>=10)break}return out;
  }
  ...
  function drawSecurityFx(ctx,x,y,c,t){const r=10+13*t;ctx.save();ctx.translate(x,y);ctx.rotate(Math.PI/4);ctx.strokeStyle=colorMix(c,.25*(1-t));ctx.lineWidth=1.2;ctx.strokeRect(-r/2,-r/2,r,r);ctx.restore();dirty(x,y,30)}
  function drawConflictFx(ctx,e,c,t){const q=ellipseFor(e);if(!q)return;const pulse=.94+Math.sin(t*Math.PI*2)*.035;ctx.save();ctx.beginPath();ctx.ellipse(q.x,q.y,q.rx*pulse,q.ry*pulse,0,0,Math.PI*2);ctx.strokeStyle=colorMix(c,.11);ctx.lineWidth=1.2;ctx.stroke();ctx.restore();dirty(q.x,q.y,Math.max(q.rx,q.ry)+6)}
