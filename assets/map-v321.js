...
(() => {
  ...
  const s={world:null,projection:null,path:null,zoom:null,transform:d3.zoomIdentity,events:[],visible:[],mode:'overview',selectedId:null,hoverId:null,selectedPlace:null,baseGroup:null,svg:null,canvas:null,fx:null,ctx:null,fxctx:null,dpr:1,hit:[],onSelect:null,onHover:null,staticRaf:0,lastStatic:0,fxRaf:0,lastFx:0,phase:0,reduce:false,fxDirty:[],animCache:[],ambient:true,motion:true,dayTimer:0,lastMode:'',palette:{}};
  const $=id=>document.getElementById(id);
  function css(name){return getComputedStyle(document.documentElement).getPropertyValue(`--${name}`).trim()}
  function refreshPalette(){for(const k of ['accent','coral','cyan','gold','mint','violet','panel','ink'])s.palette[k]=css(k)}
  function color(e){return s.palette[CAT[e.category]||'accent']||'#5d62d7'}
  ...
  function visibleFor(events,mode){
    ...
    if(mode==='conflict')return[...conflicts,...pick(live.filter(e=>e.category==='security'),{security:mobile?24:60},mobile?24:60)];
    if(mode==='hazards')return pick(live.filter(e=>e.category!=='security'),mobile?{weather:7,earthquake:8,storm:6,wildfire:5,flood:4,volcano:3,spaceweather:3,space:2,'*':2}:{weather:20,earthquake:18,storm:14,wildfire:12,flood:8,volcano:7,spaceweather:6,space:4,'*':5},mobile?42:120);
    if(mode==='signals')return[...conflicts,...live.slice(0,mobile?90:280)];
    ...
    return[...refs,...pick(hero,mobile?{weather:4,earthquake:5,storm:4,wildfire:4,security:5,space:2,spaceweather:2,flood:2,volcano:2,'*':2}:{weather:7,earthquake:10,storm:7,wildfire:7,security:9,space:4,spaceweather:4,flood:3,volcano:3,'*':3},mobile?34:64)];
  }
  function init({onSelect,onHover}={}){
    ...
    s.onSelect=onSelect;s.onHover=onHover;s.reduce=matchMedia('(prefers-reduced-motion: reduce)').matches;refreshPalette();
    setupZoom();bindPointer();resize();loadWorld();startDayNight();startEffects();return api;
  }
  async function loadWorld(){
    try{const d=await fetch('./data/world-v321.geojson',{cache:'force-cache'});if(!d.ok)throw new Error('world');s.world=await d.json()}catch(e){s.world=null}
  ...
  function resize(){
    ...
    if(w<=820){
      const top=Math.max(245,h*.18),bottom=Math.min(h*.66,top+470);
      s.projection=d3.geoEqualEarth().fitExtent([[8,top],[w-8,bottom]],{type:'Sphere'});
      s.projection.scale(s.projection.scale()*1.08);
    }else{
  ...
  function drawBase(){
    ...
    if(s.world)g.selectAll('.country').data(s.world.features).join('path').attr('class','country').attr('d',s.path).style('fill-opacity',d=>.91+(Math.abs(hash(d.properties?.name||''))%6)*.014);
    g.append('path').attr('class','night-shade');
    g.append('path').attr('class','terminator');
    ...
    g.selectAll('.continent-label').data(labels).join('text').attr('class','continent-label').attr('x',d=>s.projection([d[1],d[2]])[0]).attr('y',d=>s.projection([d[1],d[2]])[1]).text(d=>d[0]);
    updateDayNight();applyTransform();
  }
  function solarPoint(date=new Date()){
    const y=date.getUTCFullYear(),start=Date.UTC(y,0,0),today=Date.UTC(y,date.getUTCMonth(),date.getUTCDate());
    const day=Math.max(1,Math.round((today-start)/86400000)),mins=date.getUTCHours()*60+date.getUTCMinutes()+date.getUTCSeconds()/60;
    const gamma=2*Math.PI/365*(day-1+(mins/60-12)/24);
    ...
    let lon=(720-mins-eq)/4;lon=((lon+540)%360)-180;return{lon,lat:dec*180/Math.PI};
  }
  function updateDayNight(){
    if(!s.baseGroup||!s.path)return;const sun=solarPoint(),anti=[((sun.lon+180+540)%360)-180,-sun.lat];
    const night=d3.geoCircle().center(anti).radius(90).precision(2)();
    s.baseGroup.select('.night-shade').datum(night).attr('d',s.path).style('display',s.ambient?null:'none');
    s.baseGroup.select('.terminator').datum(night).attr('d',s.path).style('display',s.ambient?null:'none');
  }
  function startDayNight(){if(s.dayTimer)clearInterval(s.dayTimer);updateDayNight();s.dayTimer=setInterval(()=>{if(!document.hidden)updateDayNight()},60000)}
  function setupZoom(){
  ...
  function setData(events,mode='overview',selectedId=null){const changed=s.lastMode&&s.lastMode!==mode;s.events=events||[];s.mode=mode;s.selectedId=selectedId;s.visible=visibleFor(s.events,s.mode);s.animCache=s.motion?buildAnimated():[];if(changed&&s.canvas){s.canvas.classList.add('mode-fade');setTimeout(()=>s.canvas?.classList.remove('mode-fade'),40)}s.lastMode=mode;redraw(true);return s.visible}
  function setTheme(){refreshPalette();drawBase();redraw(true)}
  function setAmbient(on){s.ambient=!!on;updateDayNight()}
  function setMotion(on){s.motion=!!on;s.animCache=s.motion?buildAnimated():[];if(!s.motion)clearFull(s.fxctx);redrawEffects(true)}
  ...
  function redrawStatic(){
    ...
    for(const e of signals){const p=toScreen(e);if(!p||p[0]<-35||p[0]>innerWidth+35||p[1]<-35||p[1]>innerHeight+35)continue;drawMarker(ctx,e,p[0],p[1]);s.hit.push({e,x:p[0],y:p[1],r:innerWidth<=820?20:14})}
  ...
  function drawPlace(ctx,x,y,place){
    const text=`${place.name}`;ctx.save();ctx.beginPath();ctx.arc(x,y,5,0,Math.PI*2);ctx.fillStyle=s.palette.ink;ctx.fill();ctx.beginPath();ctx.arc(x,y,12,0,Math.PI*2);ctx.strokeStyle=s.palette.accent;ctx.lineWidth=2;ctx.stroke();
    const fs=innerWidth>2600?15:13;ctx.font=`650 ${fs}px -apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif`;const w=ctx.measureText(text).width;const bx=x+17,by=y-15;ctx.fillStyle=s.palette.panel;roundRect(ctx,bx-8,by-fs-5,w+16,fs+14,10);ctx.fill();ctx.strokeStyle=colorMix(s.palette.ink,.12);ctx.lineWidth=1;ctx.stroke();ctx.fillStyle=s.palette.ink;ctx.fillText(text,bx,by);ctx.restore();
  }
  function roundRect(ctx,x,y,w,h,r){const rr=Math.min(r,w/2,h/2);ctx.beginPath();ctx.moveTo(x+rr,y);ctx.arcTo(x+w,y,x+w,y+h,rr);ctx.arcTo(x+w,y+h,x,y+h,rr);ctx.arcTo(x,y+h,x,y,rr);ctx.arcTo(x,y,x+w,y,rr);ctx.closePath()}
  function drawMarker(ctx,e,x,y){
    const c=color(e),sev=e.severity||1,r=sev>=5?8.5:sev>=4?7.5:sev>=3?6.5:5.4,selected=e.id===s.selectedId,hovered=e.id===s.hoverId;ctx.save();
    if(s.selectedId&&!selected)ctx.globalAlpha=.66;
    ctx.beginPath();ctx.arc(x,y,r+2.6,0,Math.PI*2);ctx.fillStyle=colorMix(s.palette.panel,.92);ctx.fill();
    if(sev>=4||hovered||selected){ctx.beginPath();ctx.arc(x,y,r+(selected?9:hovered?6:5.5),0,Math.PI*2);ctx.fillStyle=colorMix(c,selected?.16:hovered?.12:.09);ctx.fill()}
    ctx.strokeStyle=c;ctx.fillStyle=c;ctx.lineWidth=sev>=4?2.2:1.65;
    if(e.category==='security'){ctx.translate(x,y);ctx.rotate(Math.PI/4);ctx.fillRect(-r*.70,-r*.70,r*1.40,r*1.40)}
    else if(e.category==='wildfire'){ctx.beginPath();ctx.moveTo(x,y-r);ctx.lineTo(x+r*.92,y+r*.82);ctx.lineTo(x-r*.92,y+r*.82);ctx.closePath();ctx.fill()}
    else if(e.category==='earthquake'){ctx.beginPath();ctx.arc(x,y,r,0,Math.PI*2);ctx.stroke();ctx.beginPath();ctx.arc(x,y,Math.max(2.35,r*.36),0,Math.PI*2);ctx.fill()}
    else if(e.category==='space'||e.category==='spaceweather')star(ctx,x,y,r+1,c);
    else{ctx.beginPath();ctx.arc(x,y,r,0,Math.PI*2);ctx.fill()}
    ctx.restore();
    if(hovered||selected){ctx.save();ctx.beginPath();ctx.arc(x,y,r+(selected?11:8),0,Math.PI*2);ctx.strokeStyle=selected?colorMix(s.palette.ink,.72):colorMix(c,.55);ctx.lineWidth=selected?1.7:1.25;ctx.stroke();ctx.restore()}
  }
  ...
  function buildAnimated(){
    ...
    const cfg={
      ...
      earthquake:{min:2,cap:4},wildfire:{min:2,cap:3},volcano:{min:2,cap:2},security:{min:2,cap:3},spaceweather:{min:2,cap:2},space:{min:1,cap:2}
    };
  ...
  function startEffects(){if(s.reduce)return;const loop=ts=>{s.fxRaf=requestAnimationFrame(loop);if(document.hidden||!s.motion||ts-s.lastFx<33)return;s.lastFx=ts;s.phase=(ts%12000)/12000;redrawEffects(false)};s.fxRaf=requestAnimationFrame(loop)}
  ...
  function redrawEffects(forceFull=false){
    if(!s.fxctx)return;if(forceFull){clearFull(s.fxctx);s.fxDirty=[]}else clearDirty();if(s.reduce||!s.motion||s.mode==='signals')return;
    ...
    for(const e of s.animCache){const p=toScreen(e);if(!p||p[0]<-100||p[0]>innerWidth+100||p[1]<-100||p[1]>innerHeight+100)continue;const seed=(Math.abs(hash(e.id))%1000)/1000,t=(s.phase+seed)%1,c=color(e);
      ...
      else if(e.category==='wildfire'||e.category==='volcano')drawFireFx(ctx,p[0],p[1],c,t,seed);
      else if(e.category==='security')drawSecurityFx(ctx,p[0],p[1],c,t);
      else if(e.category==='spaceweather'||e.category==='space')drawSpaceWeatherFx(ctx,p[0],p[1],c,t);
      else if(e.category==='conflict')drawConflictFx(ctx,e,c,t);
    }
    const selected=s.selectedId?s.visible.find(e=>e.id===s.selectedId):null;if(selected)drawSelectedFx(ctx,selected);
    ctx.restore();
  }
  ...
  function drawSelectedFx(ctx,e){if(e.category==='conflict'){const q=fieldFor(e);if(!q)return;ctx.save();organicPath(ctx,e,q.x,q.y,q.r*1.04,s.phase*Math.PI);ctx.strokeStyle=colorMix(color(e),.34);ctx.lineWidth=1.8;ctx.stroke();ctx.restore();dirty(q.x,q.y,q.r+18);return}const p=toScreen(e);if(!p)return;const t=(s.phase*1.35)%1,r=15+10*t;ctx.beginPath();ctx.arc(p[0],p[1],r,0,Math.PI*2);ctx.strokeStyle=colorMix(color(e),.34*(1-t));ctx.lineWidth=1.5;ctx.stroke();dirty(p[0],p[1],34)}
  function hash(str){let h=0;for(let i=0;i<String(str).length;i++)h=((h<<5)-h)+str.charCodeAt(i)|0;return h}
  function bindPointer(){const node=$('worldMap');node.addEventListener('mousemove',ev=>{const h=nearest(ev.clientX,ev.clientY),id=h?.e?.id||null;if(id!==s.hoverId){s.hoverId=id;scheduleStatic()}node.classList.toggle('is-hovering',!!h);s.onHover?.(h?.e||null,ev)});node.addEventListener('mouseleave',()=>{if(s.hoverId){s.hoverId=null;scheduleStatic()}node.classList.remove('is-hovering');s.onHover?.(null,null)});node.addEventListener('click',ev=>{const h=nearest(ev.clientX,ev.clientY);if(h)s.onSelect?.(h.e)})}
  ...
  const api={init,setData,setTheme,setAmbient,setMotion,focus,focusView,resetWorld,setPlace,resize,getVisible:()=>s.visible,getTransform:()=>s.transform,stopEffects};
